...
def _convert_fal_kontext_lora_to_diffusers(original_state_dict):
    ...
    # Global embedder keys (time_in, vector_in, txt_in, img_in, guidance_in) may appear
    # without the base_model.model. prefix in some fal-kontext LoRAs.
    for lora_key in ["lora_A", "lora_B"]:
        for src, dst in [
            ...
            (f"guidance_in.in_layer.{lora_key}.weight", f"time_text_embed.guidance_embedder.linear_1.{lora_key}.weight"),
            (f"guidance_in.out_layer.{lora_key}.weight", f"time_text_embed.guidance_embedder.linear_2.{lora_key}.weight"),
        ]:
